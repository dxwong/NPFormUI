...
{
    public class DataGridViewFilter : DataGridView
    {
        ...
        string StrFilter = "";
        string ButtonCtrlText = "确定";
        string ClearFilterCtrlText = "清空";
        string CheckCtrlAllText = "全部";
        ...
        {
            int widthTool = GetWhithColumn(e.ColumnIndex) + 30;
            if (widthTool < 110) widthTool = 60;
            ...
            textBoxCtrl.AutoSize = false;
            textBoxCtrl.Size = new System.Drawing.Size(widthTool, 18);
            textBoxCtrl.TextChanged -= textBoxCtrl_TextChanged;
            textBoxCtrl.TextChanged += textBoxCtrl_TextChanged;

            DateTimeCtrl.Size = new System.Drawing.Size(widthTool, 23);
            DateTimeCtrl.Format = DateTimePickerFormat.Custom;
            DateTimeCtrl.CustomFormat = "yyyyMMdd";
            DateTimeCtrl.TextChanged -= DateTimeCtrl_TextChanged;
            DateTimeCtrl.TextChanged += DateTimeCtrl_TextChanged;

            //CheckCtrl.BorderStyle = BorderStyle.None;
            ...
            ApplyButtonCtrl.Size = new System.Drawing.Size(widthTool, 23);
            ApplyButtonCtrl.Click -= ApplyButtonCtrl_Click;
            ApplyButtonCtrl.Click += ApplyButtonCtrl_Click;

            //ApplyButtonCtrl.Left = 70;
            ClearFilterCtrl.Text = ClearFilterCtrlText;
            ClearFilterCtrl.Size = new System.Drawing.Size(widthTool, 23);
        ...
        private List<string> GetDataColumns(int e)
        {
            ...
            {
                string Value = row.Cells[e].Value?.ToString();
        ...
        private void GetChkFilter(string key = "")
        {
            ...
            foreach (string ValueCell in GetDataColumns(columnIndex))
            {
                ...
                if (index == -1)
                {
                    CheckList.Add(new FilterStatus { valueString = ValueCell, check =false });
                }
            }

            CheckCtrl.Items.Add(CheckCtrlAllText, CheckState.Indeterminate);

            CheckList = CheckList.Where(t => t.valueString.Contains(key)).ToList();
            switch (this.Columns[columnIndex].ValueType.ToString())
            {
                case "System.Int32":
                ...
                case "System.DateTime":
                    ...
                    {
                        if (val.check == true)
                            CheckCtrl.Items.Add(DateTime.Parse(val.valueString).ToString("yyyyMMdd"), CheckState.Checked);
                        else
                            CheckCtrl.Items.Add(DateTime.Parse(val.valueString).ToString("yyyyMMdd"), CheckState.Unchecked);
                    }
                    break;
                default:
